...
class SimpleDataset:
    def __init__(self, transform, target_transform=identity, split=None):
        ...
        self.d = ImageFolder(configs.places_path, transform=transform, target_transform=target_transform)
        self.split = split
...
class SetDataset:
    def __init__(self, batch_size, transform, split=None):
        self.d = ImageFolder(configs.places_path, transform=transform)
        self.split = split
        if split is not None:
            print("Using Split: ", split)
            self.d = construct_subset(self.d, split)
